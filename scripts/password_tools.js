...
export async function storeAesKeyEncrypted(aesKey) {
  ...
  const encryptedAesKey = await encryptWithAES(aesKey, derivedKey); // Chiffre la clé AES avec la clé dérivée
  ...
  export async function encryptAesKey(aesKey, derivedKey) {
    ...
    const encryptedAesKeyString = Array.from(new Uint8Array(encryptedAesKey))
      .map(byte => byte.toString(16).padStart(2, '0'))
      .join('');
  ...
  export async function getAesKey() {
    ...
    if (encryptedAesKeyString) {
      const decryptedAesKey = await decryptWithAESKey(encryptedAesKeyString, derivedKey); // Déchiffre la clé AES chiffrée
      const encoder = new TextEncoder();
      const encodedDecryptedAesKey = encoder.encode(decryptedAesKey);
      const decryptedAesKeyBytes =  new Uint8Array(encodedDecryptedAesKey);
      const aesKey = await window.crypto.subtle.importKey(
        'raw',
        decryptedAesKeyBytes, //tableau de bytes
        {
          name: 'AES-CBC'
        },
        false,
